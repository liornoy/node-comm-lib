...
func GetIngressEndpointSlices(cs *client.ClientSet) ([]EndpointSliceInfo, error) {
	var (
...
func ToComDetails(cs *client.ClientSet, epSlicesInfo []EndpointSliceInfo) ([]types.ComDetails, error) {
	ComDetails := make([]types.ComDetails, 0)
	for _, epSliceInfo := range epSlicesInfo {
		cds, err := toComDetails(cs, epSliceInfo)
		if err != nil {
			return nil, err
		}
